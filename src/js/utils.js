  ...
  return `
    ...
    <div class="userinfo">
      ...
      <div class="level">
        ...
        <div class="exp">${info.level.current_exp} / ${info.level.next_exp}</div>
      </div>
      <div class="lv-progress divider">
        <div class="lv-percent" style="width:${(info.level.current_exp / info.level.next_exp).toFixed(2) * 100}%"></div>
      </div>
      <div class="coins">
        <a href="javascript:;" title="${info.mobile == 1 ? '已绑定' : '未绑定'}" class="phone">
          <i class="iconfont-new-home icon-bind-phone"></i>
          <i class="iconfont-new-home ${info.mobile == 1 ? 'icon-ok' : ''}"></i>
        </a>
        <a href="javascript:;" title="${info.email == 1 ? '已绑定' : '未绑定'}" class="e-mail">
              ...
              <div class="anchor">
                ...
                ${
                  info.verified === -1
                    ? ''
                    : `<svg class="verified">
                        <use xlink:href="${imgsrc}/bibo.svg#verified${info.verified === 0 ? '' : '2'}"></use>
                      </svg>`
                }
              </div>
              <div class="ancinfo">
...
const Column = (info) => {
  return `
    <div class="article-card">
      ...
      <div class="content">
        <a href="${info.link}" target="_blank" title="${info.title}" class="title">${info.title}</a>
        <a href="${info.uplink}" target="_blank" class="up">
        <i class="bilifont bili-icon_xinxi_UPzhu"></i>
        ${info.up}
        </a>
      ...
      return `
          <li>
            <a href="${info.link}" target="_blank" class="item">
              <span class="number ${info.rank <= 3 ? 'top' : ''}">${info.rank}</span>
              <div class="anchor">
    ...
    case 'video':
      return `
          <li class="${info.rank === 1 ? 'top1' : 'top'}">
            <a href="${info.link}" target="_blank" title=${info.title} class="item">
              <span class="number ${info.rank <= 3 ? 'tops' : ''}">${info.rank}</span>
        ${
          info.rank === 1
            ? ` <div class="title">
                  <div class="cover">
            ...
            <div class="detail-card">
              ...
              <ul  class="count">
                <li><i class="bilifont bili-icon_shipin_bofangshu"></i><span>${info.play}</span></li>
                <li><i class="bilifont bili-icon_shipin_danmushu"></i><span>${info.danmaku}</span></li>
                <li><i class="bilifont bili-icon_shipin_shoucangshu"></i><span>${info.favor}</span></li>
                <li><i class="bilifont bili-icon_shipin_yingbishu"></i><span>${info.coin}</span>
                </li>
              </ul>
            </div>
          </li>`
        ...
        ${
          info.rank === 1
            ...
            <a href="${info.link}" target="_blank" title=${info.title} class="item">
              ...
              <div class="title">
              <div class="cover">
                <img src="${handleImgUrl(info.cover, { height: 150 })}" alt="${info.title}">
              </div>
              <div class="txt">
              <p class="vdname">${info.title}</p>
              <span>${info.tag}</span>
            ...
            : `<li class="top no-preview">
                <a href="${info.link}" target="_blank" title=${info.title} class="item">
                  <span class="number ${info.rank <= 3 ? 'tops' : ''}">${info.rank}</span>
                  <p class="vdname">${info.title} <span>更新至${info.update}</span></p>
                </a>
              </li>`
        }`
    case 'column':
      return `
        ...
        ${
          info.rank === 1
            ? ` <div class="title">
                  <div class="cover">
                    <img src="${handleImgUrl(info.cover)}" alt="${info.title}" />
                  </div>
                  <div class="txt">
                    <p class="vdname">${info.title}</p>
                    <p class="score">综合得分：${info.score}</p>
...
const adjustVideos = function adjustVideos() {
  ...
  videoArea.forEach((area) => {
    ...
    let maxCol = Math.floor(parseFloat(areaSize.width) / parseFloat(videoSize.width))
    let maxRow = Math.floor(parseFloat(areaSize.height) / parseFloat(videoSize.height))
...
const adjustContents = function adjustContents(box, content, content2) {
  ...
  boxEle.forEach((area) => {
    ...
    let maxCol = Math.floor(parseFloat(areaSize.width) / parseFloat(contentSize.width))
    let maxRow = Math.floor(parseFloat(areaSize.height) / parseFloat(contentSize.height))
    let max = maxCol * maxRow
