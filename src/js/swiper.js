...
export class Swiper {
  ...
  render(data, styles) {
    ...
    Promise.all(imgQue)
      .then(() => {
        ...
        this.pageBox.style.width =
          this.imgNum * (styles.pageWidth + styles.pageMargin * 2) + 'px';
  ...
  swipe(tarIndex) {
    ...
    this.index =
      tarIndex < 0 ? this.imgNum - 1 : tarIndex > this.imgNum ? 1 : tarIndex;
...
export class TopSwiper {
  ...
  swipe = (tarIndex) => {
    ...
    this.title.innerHTML = `${
      this.data[curIndex].isad ? '<i class="ad-icon"></i>' : ''
    }${this.data[curIndex].title}`;
  };
  bindEvent = () => {
