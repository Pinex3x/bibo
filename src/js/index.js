...
~(function main() {
  ...
  const changeCurrent = () => {
    let currentSection = [].find.call(sections, (section) => section.getBoundingClientRect().top >= 0) || sections[0]
  ...
  const getSortItem = (tag) => {
    return [].find.call(sortItems, (item) => item.getAttribute('data-name') === tag)
  }
  ...
  const move = (e) => {
    ...
    tarIndex = utils.range(movingIndex + Math.round(moveY / itemH, 0, sortItems.length - 1))
    ...
    const change = () => {
      ;[].forEach.call(sortItems, (item, index) => {
        ...
        if (index >= min && index <= max) item.style = `transform:translate3d(0px,${itemH * flag}px,0px)`
        else item.style = `transform:translate3d(0px,0px,0px)`
      })
      movingItem.style = `transform:translate3d(0px,${e.clientY - startY}px,0px)`
    }
  ...
  const preview = async (e) => {
    let tar = e.target
    if (!tar.classList.contains('videocut') || tar.classList.contains('loaded')) return
    ...
    Promise.all([
      ...
      .then(async (res) => {
        ...
        tar.style.backgroundSize = `${size.width * img_x_len}px ${size.height * img_y_len}px`
        ...
        tar.addEventListener('mousemove', (e) => {
          ...
          let curTime = index.find((time, i) => i === index.length - 1 || (sec <= time && time < index[i + 1]))
          changeKeyframe(curTime, { img_x_len, img_y_len, index, image, size }, tar)
        })
        // 弹幕滚动
  ...
  const changeKeyframe = async (curTime = 0, data, tar) => {
    ...
    tar.style.backgroundPosition = `${-position.x * size.width}px ${-position.y * size.height}px `
  }
  ...
  const danmuDisplay = (danmus, line, size, delay = 1000) => {
    ...
    const start = () => {
      ...
      if (runningTimer.length) {
        runningTimer = runningTimer.map((timer, index) => setInterval(runningFn[index], delay))
      }
  ...
  document.addEventListener('click', (e) => {
    ...
    if (path.includes(changeBtn)) {
      ...
      axios.get(rcmdAPI, { params: { fresh_type: 3 } }).then((res) => renderRcmd(res))
    }
  })
